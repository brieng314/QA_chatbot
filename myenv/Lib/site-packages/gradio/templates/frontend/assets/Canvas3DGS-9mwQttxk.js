import{p as Ut,d as ct,e as Et}from"./i18n-CLA-aXfX.js";import{p as St,ac as Jt,j as Qt,m as At,l as nt,d as it,i as Y,a as Vt,f as Zt,b as mt,c as ut,g as Ft}from"./index-zQcUQ9m1.js";class R{constructor(t=0,e=0,A=0){this.x=t,this.y=e,this.z=A}equals(t){return!(this.x!==t.x||this.y!==t.y||this.z!==t.z)}add(t){return typeof t=="number"?new R(this.x+t,this.y+t,this.z+t):new R(this.x+t.x,this.y+t.y,this.z+t.z)}subtract(t){return typeof t=="number"?new R(this.x-t,this.y-t,this.z-t):new R(this.x-t.x,this.y-t.y,this.z-t.z)}multiply(t){return typeof t=="number"?new R(this.x*t,this.y*t,this.z*t):t instanceof R?new R(this.x*t.x,this.y*t.y,this.z*t.z):new R(this.x*t.buffer[0]+this.y*t.buffer[4]+this.z*t.buffer[8]+t.buffer[12],this.x*t.buffer[1]+this.y*t.buffer[5]+this.z*t.buffer[9]+t.buffer[13],this.x*t.buffer[2]+this.y*t.buffer[6]+this.z*t.buffer[10]+t.buffer[14])}divide(t){return typeof t=="number"?new R(this.x/t,this.y/t,this.z/t):new R(this.x/t.x,this.y/t.y,this.z/t.z)}cross(t){const e=this.y*t.z-this.z*t.y,A=this.z*t.x-this.x*t.z,Q=this.x*t.y-this.y*t.x;return new R(e,A,Q)}dot(t){return this.x*t.x+this.y*t.y+this.z*t.z}lerp(t,e){return new R(this.x+(t.x-this.x)*e,this.y+(t.y-this.y)*e,this.z+(t.z-this.z)*e)}min(t){return new R(Math.min(this.x,t.x),Math.min(this.y,t.y),Math.min(this.z,t.z))}max(t){return new R(Math.max(this.x,t.x),Math.max(this.y,t.y),Math.max(this.z,t.z))}getComponent(t){switch(t){case 0:return this.x;case 1:return this.y;case 2:return this.z;default:throw new Error(`Invalid component index: ${t}`)}}minComponent(){return this.x<this.y&&this.x<this.z?0:this.y<this.z?1:2}maxComponent(){return this.x>this.y&&this.x>this.z?0:this.y>this.z?1:2}magnitude(){return Math.sqrt(this.x*this.x+this.y*this.y+this.z*this.z)}distanceTo(t){return Math.sqrt((this.x-t.x)**2+(this.y-t.y)**2+(this.z-t.z)**2)}normalize(){const t=this.magnitude();return new R(this.x/t,this.y/t,this.z/t)}flat(){return[this.x,this.y,this.z]}clone(){return new R(this.x,this.y,this.z)}toString(){return`[${this.flat().join(", ")}]`}static One(t=1){return new R(t,t,t)}}class f{constructor(t=0,e=0,A=0,Q=1){this.x=t,this.y=e,this.z=A,this.w=Q}equals(t){return!(this.x!==t.x||this.y!==t.y||this.z!==t.z||this.w!==t.w)}normalize(){const t=Math.sqrt(this.x*this.x+this.y*this.y+this.z*this.z+this.w*this.w);return new f(this.x/t,this.y/t,this.z/t,this.w/t)}multiply(t){const e=this.w,A=this.x,Q=this.y,i=this.z,n=t.w,U=t.x,F=t.y,s=t.z;return new f(e*U+A*n+Q*s-i*F,e*F-A*s+Q*n+i*U,e*s+A*F-Q*U+i*n,e*n-A*U-Q*F-i*s)}inverse(){const t=this.x*this.x+this.y*this.y+this.z*this.z+this.w*this.w;return new f(-this.x/t,-this.y/t,-this.z/t,this.w/t)}apply(t){const e=new f(t.x,t.y,t.z,0),A=new f(-this.x,-this.y,-this.z,this.w),Q=this.multiply(e).multiply(A);return new R(Q.x,Q.y,Q.z)}flat(){return[this.x,this.y,this.z,this.w]}clone(){return new f(this.x,this.y,this.z,this.w)}static FromEuler(t){const e=t.x/2,A=t.y/2,Q=t.z/2,i=Math.cos(A),n=Math.sin(A),U=Math.cos(e),F=Math.sin(e),s=Math.cos(Q),C=Math.sin(Q);return new f(i*F*s+n*U*C,n*U*s-i*F*C,i*U*C-n*F*s,i*U*s+n*F*C)}toEuler(){const t=2*(this.w*this.x+this.y*this.z),e=1-2*(this.x*this.x+this.y*this.y),A=Math.atan2(t,e);let Q;const i=2*(this.w*this.y-this.z*this.x);Math.abs(i)>=1?Q=Math.sign(i)*Math.PI/2:Q=Math.asin(i);const n=2*(this.w*this.z+this.x*this.y),U=1-2*(this.y*this.y+this.z*this.z),F=Math.atan2(n,U);return new R(A,Q,F)}static FromMatrix3(t){const e=t.buffer,A=e[0]+e[4]+e[8];let Q,i,n,U;if(A>0){const F=.5/Math.sqrt(A+1);U=.25/F,Q=(e[7]-e[5])*F,i=(e[2]-e[6])*F,n=(e[3]-e[1])*F}else if(e[0]>e[4]&&e[0]>e[8]){const F=2*Math.sqrt(1+e[0]-e[4]-e[8]);U=(e[7]-e[5])/F,Q=.25*F,i=(e[1]+e[3])/F,n=(e[2]+e[6])/F}else if(e[4]>e[8]){const F=2*Math.sqrt(1+e[4]-e[0]-e[8]);U=(e[2]-e[6])/F,Q=(e[1]+e[3])/F,i=.25*F,n=(e[5]+e[7])/F}else{const F=2*Math.sqrt(1+e[8]-e[0]-e[4]);U=(e[3]-e[1])/F,Q=(e[2]+e[6])/F,i=(e[5]+e[7])/F,n=.25*F}return new f(Q,i,n,U)}static FromAxisAngle(t,e){const A=e/2,Q=Math.sin(A),i=Math.cos(A);return new f(t.x*Q,t.y*Q,t.z*Q,i)}static LookRotation(t){const e=new R(0,0,1),A=e.dot(t);if(Math.abs(A- -1)<1e-6)return new f(0,1,0,Math.PI);if(Math.abs(A-1)<1e-6)return new f;const Q=Math.acos(A),i=e.cross(t).normalize();return f.FromAxisAngle(i,Q)}toString(){return`[${this.flat().join(", ")}]`}}class at{constructor(){const t=new Map;this.addEventListener=(e,A)=>{t.has(e)||t.set(e,new Set),t.get(e).add(A)},this.removeEventListener=(e,A)=>{t.has(e)&&t.get(e).delete(A)},this.hasEventListener=(e,A)=>t.has(e)?t.get(e).has(A):!1,this.dispatchEvent=e=>{if(t.has(e.type))for(const A of t.get(e.type))A(e)}}}class _{constructor(t=1,e=0,A=0,Q=0,i=0,n=1,U=0,F=0,s=0,C=0,d=1,B=0,r=0,I=0,h=0,a=1){this.buffer=[t,e,A,Q,i,n,U,F,s,C,d,B,r,I,h,a]}equals(t){if(this.buffer.length!==t.buffer.length)return!1;if(this.buffer===t.buffer)return!0;for(let e=0;e<this.buffer.length;e++)if(this.buffer[e]!==t.buffer[e])return!1;return!0}multiply(t){const e=this.buffer,A=t.buffer;return new _(A[0]*e[0]+A[1]*e[4]+A[2]*e[8]+A[3]*e[12],A[0]*e[1]+A[1]*e[5]+A[2]*e[9]+A[3]*e[13],A[0]*e[2]+A[1]*e[6]+A[2]*e[10]+A[3]*e[14],A[0]*e[3]+A[1]*e[7]+A[2]*e[11]+A[3]*e[15],A[4]*e[0]+A[5]*e[4]+A[6]*e[8]+A[7]*e[12],A[4]*e[1]+A[5]*e[5]+A[6]*e[9]+A[7]*e[13],A[4]*e[2]+A[5]*e[6]+A[6]*e[10]+A[7]*e[14],A[4]*e[3]+A[5]*e[7]+A[6]*e[11]+A[7]*e[15],A[8]*e[0]+A[9]*e[4]+A[10]*e[8]+A[11]*e[12],A[8]*e[1]+A[9]*e[5]+A[10]*e[9]+A[11]*e[13],A[8]*e[2]+A[9]*e[6]+A[10]*e[10]+A[11]*e[14],A[8]*e[3]+A[9]*e[7]+A[10]*e[11]+A[11]*e[15],A[12]*e[0]+A[13]*e[4]+A[14]*e[8]+A[15]*e[12],A[12]*e[1]+A[13]*e[5]+A[14]*e[9]+A[15]*e[13],A[12]*e[2]+A[13]*e[6]+A[14]*e[10]+A[15]*e[14],A[12]*e[3]+A[13]*e[7]+A[14]*e[11]+A[15]*e[15])}clone(){const t=this.buffer;return new _(t[0],t[1],t[2],t[3],t[4],t[5],t[6],t[7],t[8],t[9],t[10],t[11],t[12],t[13],t[14],t[15])}determinant(){const t=this.buffer;return t[12]*t[9]*t[6]*t[3]-t[8]*t[13]*t[6]*t[3]-t[12]*t[5]*t[10]*t[3]+t[4]*t[13]*t[10]*t[3]+t[8]*t[5]*t[14]*t[3]-t[4]*t[9]*t[14]*t[3]-t[12]*t[9]*t[2]*t[7]+t[8]*t[13]*t[2]*t[7]+t[12]*t[1]*t[10]*t[7]-t[0]*t[13]*t[10]*t[7]-t[8]*t[1]*t[14]*t[7]+t[0]*t[9]*t[14]*t[7]+t[12]*t[5]*t[2]*t[11]-t[4]*t[13]*t[2]*t[11]-t[12]*t[1]*t[6]*t[11]+t[0]*t[13]*t[6]*t[11]+t[4]*t[1]*t[14]*t[11]-t[0]*t[5]*t[14]*t[11]-t[8]*t[5]*t[2]*t[15]+t[4]*t[9]*t[2]*t[15]+t[8]*t[1]*t[6]*t[15]-t[0]*t[9]*t[6]*t[15]-t[4]*t[1]*t[10]*t[15]+t[0]*t[5]*t[10]*t[15]}invert(){const t=this.buffer,e=this.determinant();if(e===0)throw new Error("Matrix is not invertible.");const A=1/e;return new _(A*(t[5]*t[10]*t[15]-t[5]*t[11]*t[14]-t[9]*t[6]*t[15]+t[9]*t[7]*t[14]+t[13]*t[6]*t[11]-t[13]*t[7]*t[10]),A*(-t[1]*t[10]*t[15]+t[1]*t[11]*t[14]+t[9]*t[2]*t[15]-t[9]*t[3]*t[14]-t[13]*t[2]*t[11]+t[13]*t[3]*t[10]),A*(t[1]*t[6]*t[15]-t[1]*t[7]*t[14]-t[5]*t[2]*t[15]+t[5]*t[3]*t[14]+t[13]*t[2]*t[7]-t[13]*t[3]*t[6]),A*(-t[1]*t[6]*t[11]+t[1]*t[7]*t[10]+t[5]*t[2]*t[11]-t[5]*t[3]*t[10]-t[9]*t[2]*t[7]+t[9]*t[3]*t[6]),A*(-t[4]*t[10]*t[15]+t[4]*t[11]*t[14]+t[8]*t[6]*t[15]-t[8]*t[7]*t[14]-t[12]*t[6]*t[11]+t[12]*t[7]*t[10]),A*(t[0]*t[10]*t[15]-t[0]*t[11]*t[14]-t[8]*t[2]*t[15]+t[8]*t[3]*t[14]+t[12]*t[2]*t[11]-t[12]*t[3]*t[10]),A*(-t[0]*t[6]*t[15]+t[0]*t[7]*t[14]+t[4]*t[2]*t[15]-t[4]*t[3]*t[14]-t[12]*t[2]*t[7]+t[12]*t[3]*t[6]),A*(t[0]*t[6]*t[11]-t[0]*t[7]*t[10]-t[4]*t[2]*t[11]+t[4]*t[3]*t[10]+t[8]*t[2]*t[7]-t[8]*t[3]*t[6]),A*(t[4]*t[9]*t[15]-t[4]*t[11]*t[13]-t[8]*t[5]*t[15]+t[8]*t[7]*t[13]+t[12]*t[5]*t[11]-t[12]*t[7]*t[9]),A*(-t[0]*t[9]*t[15]+t[0]*t[11]*t[13]+t[8]*t[1]*t[15]-t[8]*t[3]*t[13]-t[12]*t[1]*t[11]+t[12]*t[3]*t[9]),A*(t[0]*t[5]*t[15]-t[0]*t[7]*t[13]-t[4]*t[1]*t[15]+t[4]*t[3]*t[13]+t[12]*t[1]*t[7]-t[12]*t[3]*t[5]),A*(-t[0]*t[5]*t[11]+t[0]*t[7]*t[9]+t[4]*t[1]*t[11]-t[4]*t[3]*t[9]-t[8]*t[1]*t[7]+t[8]*t[3]*t[5]),A*(-t[4]*t[9]*t[14]+t[4]*t[10]*t[13]+t[8]*t[5]*t[14]-t[8]*t[6]*t[13]-t[12]*t[5]*t[10]+t[12]*t[6]*t[9]),A*(t[0]*t[9]*t[14]-t[0]*t[10]*t[13]-t[8]*t[1]*t[14]+t[8]*t[2]*t[13]+t[12]*t[1]*t[10]-t[12]*t[2]*t[9]),A*(-t[0]*t[5]*t[14]+t[0]*t[6]*t[13]+t[4]*t[1]*t[14]-t[4]*t[2]*t[13]-t[12]*t[1]*t[6]+t[12]*t[2]*t[5]),A*(t[0]*t[5]*t[10]-t[0]*t[6]*t[9]-t[4]*t[1]*t[10]+t[4]*t[2]*t[9]+t[8]*t[1]*t[6]-t[8]*t[2]*t[5]))}static Compose(t,e,A){const Q=e.x,i=e.y,n=e.z,U=e.w,F=Q+Q,s=i+i,C=n+n,d=Q*F,B=Q*s,r=Q*C,I=i*s,h=i*C,a=n*C,l=U*F,c=U*s,u=U*C,w=A.x,D=A.y,k=A.z;return new _((1-(I+a))*w,(B+u)*w,(r-c)*w,0,(B-u)*D,(1-(d+a))*D,(h+l)*D,0,(r+c)*k,(h-l)*k,(1-(d+I))*k,0,t.x,t.y,t.z,1)}toString(){return`[${this.buffer.join(", ")}]`}}class Nt extends Event{constructor(t){super("objectAdded"),this.object=t}}class Wt extends Event{constructor(t){super("objectRemoved"),this.object=t}}class Dt extends Event{constructor(t){super("objectChanged"),this.object=t}}class gt extends at{constructor(){super(),this.positionChanged=!1,this.rotationChanged=!1,this.scaleChanged=!1,this._position=new R,this._rotation=new f,this._scale=new R(1,1,1),this._transform=new _,this._changeEvent=new Dt(this),this.update=()=>{},this.applyPosition=()=>{this.position=new R},this.applyRotation=()=>{this.rotation=new f},this.applyScale=()=>{this.scale=new R(1,1,1)},this.raiseChangeEvent=()=>{this.dispatchEvent(this._changeEvent)}}_updateMatrix(){this._transform=_.Compose(this._position,this._rotation,this._scale)}get position(){return this._position}set position(t){this._position.equals(t)||(this._position=t,this.positionChanged=!0,this._updateMatrix(),this.dispatchEvent(this._changeEvent))}get rotation(){return this._rotation}set rotation(t){this._rotation.equals(t)||(this._rotation=t,this.rotationChanged=!0,this._updateMatrix(),this.dispatchEvent(this._changeEvent))}get scale(){return this._scale}set scale(t){this._scale.equals(t)||(this._scale=t,this.scaleChanged=!0,this._updateMatrix(),this.dispatchEvent(this._changeEvent))}get forward(){let t=new R(0,0,1);return t=this.rotation.apply(t),t}get transform(){return this._transform}}class P{constructor(t=1,e=0,A=0,Q=0,i=1,n=0,U=0,F=0,s=1){this.buffer=[t,e,A,Q,i,n,U,F,s]}equals(t){if(this.buffer.length!==t.buffer.length)return!1;if(this.buffer===t.buffer)return!0;for(let e=0;e<this.buffer.length;e++)if(this.buffer[e]!==t.buffer[e])return!1;return!0}multiply(t){const e=this.buffer,A=t.buffer;return new P(A[0]*e[0]+A[3]*e[1]+A[6]*e[2],A[1]*e[0]+A[4]*e[1]+A[7]*e[2],A[2]*e[0]+A[5]*e[1]+A[8]*e[2],A[0]*e[3]+A[3]*e[4]+A[6]*e[5],A[1]*e[3]+A[4]*e[4]+A[7]*e[5],A[2]*e[3]+A[5]*e[4]+A[8]*e[5],A[0]*e[6]+A[3]*e[7]+A[6]*e[8],A[1]*e[6]+A[4]*e[7]+A[7]*e[8],A[2]*e[6]+A[5]*e[7]+A[8]*e[8])}clone(){const t=this.buffer;return new P(t[0],t[1],t[2],t[3],t[4],t[5],t[6],t[7],t[8])}static Eye(t=1){return new P(t,0,0,0,t,0,0,0,t)}static Diagonal(t){return new P(t.x,0,0,0,t.y,0,0,0,t.z)}static RotationFromQuaternion(t){return new P(1-2*t.y*t.y-2*t.z*t.z,2*t.x*t.y-2*t.z*t.w,2*t.x*t.z+2*t.y*t.w,2*t.x*t.y+2*t.z*t.w,1-2*t.x*t.x-2*t.z*t.z,2*t.y*t.z-2*t.x*t.w,2*t.x*t.z-2*t.y*t.w,2*t.y*t.z+2*t.x*t.w,1-2*t.x*t.x-2*t.y*t.y)}static RotationFromEuler(t){const e=Math.cos(t.x),A=Math.sin(t.x),Q=Math.cos(t.y),i=Math.sin(t.y),n=Math.cos(t.z),U=Math.sin(t.z),F=[Q*n+i*A*U,-Q*U+i*A*n,i*e,e*U,e*n,-A,-i*n+Q*A*U,i*U+Q*A*n,Q*e];return new P(...F)}toString(){return`[${this.buffer.join(", ")}]`}}class v{constructor(t=0,e=null,A=null,Q=null,i=null){this.changed=!1,this.detached=!1,this._vertexCount=t,this._positions=e||new Float32Array(0),this._rotations=A||new Float32Array(0),this._scales=Q||new Float32Array(0),this._colors=i||new Uint8Array(0),this._selection=new Uint8Array(this.vertexCount),this.translate=n=>{for(let U=0;U<this.vertexCount;U++)this.positions[3*U+0]+=n.x,this.positions[3*U+1]+=n.y,this.positions[3*U+2]+=n.z;this.changed=!0},this.rotate=n=>{const U=P.RotationFromQuaternion(n).buffer;for(let F=0;F<this.vertexCount;F++){const s=this.positions[3*F+0],C=this.positions[3*F+1],d=this.positions[3*F+2];this.positions[3*F+0]=U[0]*s+U[1]*C+U[2]*d,this.positions[3*F+1]=U[3]*s+U[4]*C+U[5]*d,this.positions[3*F+2]=U[6]*s+U[7]*C+U[8]*d;const B=new f(this.rotations[4*F+1],this.rotations[4*F+2],this.rotations[4*F+3],this.rotations[4*F+0]),r=n.multiply(B);this.rotations[4*F+1]=r.x,this.rotations[4*F+2]=r.y,this.rotations[4*F+3]=r.z,this.rotations[4*F+0]=r.w}this.changed=!0},this.scale=n=>{for(let U=0;U<this.vertexCount;U++)this.positions[3*U+0]*=n.x,this.positions[3*U+1]*=n.y,this.positions[3*U+2]*=n.z,this.scales[3*U+0]*=n.x,this.scales[3*U+1]*=n.y,this.scales[3*U+2]*=n.z;this.changed=!0},this.serialize=()=>{const n=new Uint8Array(this.vertexCount*v.RowLength),U=new Float32Array(n.buffer),F=new Uint8Array(n.buffer);for(let s=0;s<this.vertexCount;s++)U[8*s+0]=this.positions[3*s+0],U[8*s+1]=this.positions[3*s+1],U[8*s+2]=this.positions[3*s+2],F[32*s+24+0]=this.colors[4*s+0],F[32*s+24+1]=this.colors[4*s+1],F[32*s+24+2]=this.colors[4*s+2],F[32*s+24+3]=this.colors[4*s+3],U[8*s+3+0]=this.scales[3*s+0],U[8*s+3+1]=this.scales[3*s+1],U[8*s+3+2]=this.scales[3*s+2],F[32*s+28+0]=this.rotations[4*s+0]*128+128&255,F[32*s+28+1]=this.rotations[4*s+1]*128+128&255,F[32*s+28+2]=this.rotations[4*s+2]*128+128&255,F[32*s+28+3]=this.rotations[4*s+3]*128+128&255;return n},this.reattach=(n,U,F,s,C)=>{console.assert(n.byteLength===this.vertexCount*3*4,`Expected ${this.vertexCount*3*4} bytes, got ${n.byteLength} bytes`),this._positions=new Float32Array(n),this._rotations=new Float32Array(U),this._scales=new Float32Array(F),this._colors=new Uint8Array(s),this._selection=new Uint8Array(C),this.detached=!1}}static{this.RowLength=32}static Deserialize(t){const e=t.length/v.RowLength,A=new Float32Array(3*e),Q=new Float32Array(4*e),i=new Float32Array(3*e),n=new Uint8Array(4*e),U=new Float32Array(t.buffer),F=new Uint8Array(t.buffer);for(let s=0;s<e;s++)A[3*s+0]=U[8*s+0],A[3*s+1]=U[8*s+1],A[3*s+2]=U[8*s+2],Q[4*s+0]=(F[32*s+28+0]-128)/128,Q[4*s+1]=(F[32*s+28+1]-128)/128,Q[4*s+2]=(F[32*s+28+2]-128)/128,Q[4*s+3]=(F[32*s+28+3]-128)/128,i[3*s+0]=U[8*s+3+0],i[3*s+1]=U[8*s+3+1],i[3*s+2]=U[8*s+3+2],n[4*s+0]=F[32*s+24+0],n[4*s+1]=F[32*s+24+1],n[4*s+2]=F[32*s+24+2],n[4*s+3]=F[32*s+24+3];return new v(e,A,Q,i,n)}get vertexCount(){return this._vertexCount}get positions(){return this._positions}get rotations(){return this._rotations}get scales(){return this._scales}get colors(){return this._colors}get selection(){return this._selection}clone(){return new v(this.vertexCount,new Float32Array(this.positions),new Float32Array(this.rotations),new Float32Array(this.scales),new Uint8Array(this.colors))}}class et{static{this.SH_C0=.28209479177387814}static SplatToPLY(t,e){let A=`ply
format binary_little_endian 1.0
`;A+=`element vertex ${e}
`;const Q=["x","y","z","nx","ny","nz","f_dc_0","f_dc_1","f_dc_2"];for(let a=0;a<45;a++)Q.push(`f_rest_${a}`);Q.push("opacity"),Q.push("scale_0"),Q.push("scale_1"),Q.push("scale_2"),Q.push("rot_0"),Q.push("rot_1"),Q.push("rot_2"),Q.push("rot_3");for(const a of Q)A+=`property float ${a}
`;A+=`end_header
`;const i=new TextEncoder().encode(A),n=248,U=e*n,F=new DataView(new ArrayBuffer(i.length+U));new Uint8Array(F.buffer).set(i,0);const s=new Float32Array(t),C=new Uint8Array(t),d=i.length,B=24,r=B+12+180,I=r+4,h=I+12;for(let a=0;a<e;a++){const l=s[8*a+0],c=s[8*a+1],u=s[8*a+2],w=(C[32*a+24+0]/255-.5)/this.SH_C0,D=(C[32*a+24+1]/255-.5)/this.SH_C0,k=(C[32*a+24+2]/255-.5)/this.SH_C0,N=C[32*a+24+3]/255,y=Math.log(N/(1-N)),V=Math.log(s[8*a+3+0]),z=Math.log(s[8*a+3+1]),M=Math.log(s[8*a+3+2]);let x=new f((C[32*a+28+1]-128)/128,(C[32*a+28+2]-128)/128,(C[32*a+28+3]-128)/128,(C[32*a+28+0]-128)/128);x=x.normalize();const Z=x.w,p=x.x,O=x.y,T=x.z;F.setFloat32(d+n*a+0,l,!0),F.setFloat32(d+n*a+4,c,!0),F.setFloat32(d+n*a+8,u,!0),F.setFloat32(d+n*a+B+0,w,!0),F.setFloat32(d+n*a+B+4,D,!0),F.setFloat32(d+n*a+B+8,k,!0),F.setFloat32(d+n*a+r,y,!0),F.setFloat32(d+n*a+I+0,V,!0),F.setFloat32(d+n*a+I+4,z,!0),F.setFloat32(d+n*a+I+8,M,!0),F.setFloat32(d+n*a+h+0,Z,!0),F.setFloat32(d+n*a+h+4,p,!0),F.setFloat32(d+n*a+h+8,O,!0),F.setFloat32(d+n*a+h+12,T,!0)}return F.buffer}}class st{constructor(t,e){this.min=t,this.max=e}contains(t){return t.x>=this.min.x&&t.x<=this.max.x&&t.y>=this.min.y&&t.y<=this.max.y&&t.z>=this.min.z&&t.z<=this.max.z}intersects(t){return this.max.x>=t.min.x&&this.min.x<=t.max.x&&this.max.y>=t.min.y&&this.min.y<=t.max.y&&this.max.z>=t.min.z&&this.min.z<=t.max.z}size(){return this.max.subtract(this.min)}center(){return this.min.add(this.max).divide(2)}expand(t){this.min=this.min.min(t),this.max=this.max.max(t)}permute(){const t=this.min,e=this.max;this.min=new R(Math.min(t.x,e.x),Math.min(t.y,e.y),Math.min(t.z,e.z)),this.max=new R(Math.max(t.x,e.x),Math.max(t.y,e.y),Math.max(t.z,e.z))}}class H extends gt{constructor(t=void 0){super(),this.selectedChanged=!1,this.colorTransformChanged=!1,this._selected=!1,this._colorTransforms=[],this._colorTransformsMap=new Map,this._data=t||new v,this._bounds=new st(new R(1/0,1/0,1/0),new R(-1/0,-1/0,-1/0)),this.recalculateBounds=()=>{this._bounds=new st(new R(1/0,1/0,1/0),new R(-1/0,-1/0,-1/0));for(let e=0;e<this._data.vertexCount;e++)this._bounds.expand(new R(this._data.positions[3*e],this._data.positions[3*e+1],this._data.positions[3*e+2]))},this.applyPosition=()=>{this.data.translate(this.position),this.position=new R},this.applyRotation=()=>{this.data.rotate(this.rotation),this.rotation=new f},this.applyScale=()=>{this.data.scale(this.scale),this.scale=new R(1,1,1)},this.recalculateBounds()}saveToFile(t=null,e="splat"){if(!document)return;if(!t){const U=new Date;t=`splat-${U.getFullYear()}-${U.getMonth()+1}-${U.getDate()}.${e}`}const A=this.clone();A.applyRotation(),A.applyScale(),A.applyPosition();const Q=A.data.serialize();let i;if(e==="ply"){const U=et.SplatToPLY(Q.buffer,A.data.vertexCount);i=new Blob([U],{type:"application/octet-stream"})}else i=new Blob([Q.buffer],{type:"application/octet-stream"});const n=document.createElement("a");n.download=t,n.href=URL.createObjectURL(i),n.click()}get data(){return this._data}get selected(){return this._selected}set selected(t){this._selected!==t&&(this._selected=t,this.selectedChanged=!0,this.dispatchEvent(this._changeEvent))}get colorTransforms(){return this._colorTransforms}get colorTransformsMap(){return this._colorTransformsMap}get bounds(){let t=this._bounds.center();t=t.add(this.position);let e=this._bounds.size();return e=e.multiply(this.scale),new st(t.subtract(e.divide(2)),t.add(e.divide(2)))}clone(){const t=new H(this.data.clone());return t.position=this.position.clone(),t.rotation=this.rotation.clone(),t.scale=this.scale.clone(),t}}class pt{constructor(){this._fx=1132,this._fy=1132,this._near=.1,this._far=100,this._width=512,this._height=512,this._projectionMatrix=new _,this._viewMatrix=new _,this._viewProj=new _,this._updateProjectionMatrix=()=>{this._projectionMatrix=new _(2*this.fx/this.width,0,0,0,0,-2*this.fy/this.height,0,0,0,0,this.far/(this.far-this.near),1,0,0,-(this.far*this.near)/(this.far-this.near),0),this._viewProj=this.projectionMatrix.multiply(this.viewMatrix)},this.update=(t,e)=>{const A=P.RotationFromQuaternion(e).buffer,Q=t.flat();this._viewMatrix=new _(A[0],A[1],A[2],0,A[3],A[4],A[5],0,A[6],A[7],A[8],0,-Q[0]*A[0]-Q[1]*A[3]-Q[2]*A[6],-Q[0]*A[1]-Q[1]*A[4]-Q[2]*A[7],-Q[0]*A[2]-Q[1]*A[5]-Q[2]*A[8],1),this._viewProj=this.projectionMatrix.multiply(this.viewMatrix)},this.setSize=(t,e)=>{this._width=t,this._height=e,this._updateProjectionMatrix()}}get fx(){return this._fx}set fx(t){this._fx!==t&&(this._fx=t,this._updateProjectionMatrix())}get fy(){return this._fy}set fy(t){this._fy!==t&&(this._fy=t,this._updateProjectionMatrix())}get near(){return this._near}set near(t){this._near!==t&&(this._near=t,this._updateProjectionMatrix())}get far(){return this._far}set far(t){this._far!==t&&(this._far=t,this._updateProjectionMatrix())}get width(){return this._width}get height(){return this._height}get projectionMatrix(){return this._projectionMatrix}get viewMatrix(){return this._viewMatrix}get viewProj(){return this._viewProj}}class X{constructor(t=0,e=0,A=0,Q=0){this.x=t,this.y=e,this.z=A,this.w=Q}equals(t){return!(this.x!==t.x||this.y!==t.y||this.z!==t.z||this.w!==t.w)}add(t){return typeof t=="number"?new X(this.x+t,this.y+t,this.z+t,this.w+t):new X(this.x+t.x,this.y+t.y,this.z+t.z,this.w+t.w)}subtract(t){return typeof t=="number"?new X(this.x-t,this.y-t,this.z-t,this.w-t):new X(this.x-t.x,this.y-t.y,this.z-t.z,this.w-t.w)}multiply(t){return typeof t=="number"?new X(this.x*t,this.y*t,this.z*t,this.w*t):t instanceof X?new X(this.x*t.x,this.y*t.y,this.z*t.z,this.w*t.w):new X(this.x*t.buffer[0]+this.y*t.buffer[4]+this.z*t.buffer[8]+this.w*t.buffer[12],this.x*t.buffer[1]+this.y*t.buffer[5]+this.z*t.buffer[9]+this.w*t.buffer[13],this.x*t.buffer[2]+this.y*t.buffer[6]+this.z*t.buffer[10]+this.w*t.buffer[14],this.x*t.buffer[3]+this.y*t.buffer[7]+this.z*t.buffer[11]+this.w*t.buffer[15])}dot(t){return this.x*t.x+this.y*t.y+this.z*t.z+this.w*t.w}lerp(t,e){return new X(this.x+(t.x-this.x)*e,this.y+(t.y-this.y)*e,this.z+(t.z-this.z)*e,this.w+(t.w-this.w)*e)}magnitude(){return Math.sqrt(this.x*this.x+this.y*this.y+this.z*this.z+this.w*this.w)}distanceTo(t){return Math.sqrt((this.x-t.x)**2+(this.y-t.y)**2+(this.z-t.z)**2+(this.w-t.w)**2)}normalize(){const t=this.magnitude();return new X(this.x/t,this.y/t,this.z/t,this.w/t)}flat(){return[this.x,this.y,this.z,this.w]}clone(){return new X(this.x,this.y,this.z,this.w)}toString(){return`[${this.flat().join(", ")}]`}}class ft extends gt{constructor(t=void 0){super(),this._data=t||new pt,this._position=new R(0,0,-5),this.update=()=>{this.data.update(this.position,this.rotation)},this.screenPointToRay=(e,A)=>{const Q=new X(e,A,-1,1),i=this._data.projectionMatrix.invert(),n=Q.multiply(i),U=this._data.viewMatrix.invert(),F=n.multiply(U);return new R(F.x/F.w,F.y/F.w,F.z/F.w).subtract(this.position).normalize()}}get data(){return this._data}}class kt extends at{constructor(){super(),this._objects=[],this.addObject=t=>{this.objects.push(t),this.dispatchEvent(new Nt(t))},this.removeObject=t=>{const e=this.objects.indexOf(t);if(e<0)throw new Error("Object not found in scene");this.objects.splice(e,1),this.dispatchEvent(new Wt(t))},this.findObject=t=>{for(const e of this.objects)if(t(e))return e},this.findObjectOfType=t=>{for(const e of this.objects)if(e instanceof t)return e},this.reset=()=>{const t=this.objects.slice();for(const e of t)this.removeObject(e)},this.reset()}getMergedSceneDataBuffer(t="splat"){const e=[];let A=0;for(const n of this.objects)if(n instanceof H){const U=n.clone();U.applyRotation(),U.applyScale(),U.applyPosition();const F=U.data.serialize();e.push(F),A+=U.data.vertexCount}const Q=new Uint8Array(A*v.RowLength);let i=0;for(const n of e)Q.set(n,i),i+=n.length;return t==="ply"?et.SplatToPLY(Q.buffer,A):Q.buffer}saveToFile(t=null,e="splat"){if(!document)return;if(!t){const n=new Date;t=`scene-${n.getFullYear()}-${n.getMonth()+1}-${n.getDate()}.${e}`}const A=this.getMergedSceneDataBuffer(e),Q=new Blob([A],{type:"application/octet-stream"}),i=document.createElement("a");i.download=t,i.href=URL.createObjectURL(Q),i.click()}get objects(){return this._objects}}async function Ct(E,t){const e=await fetch(E,{mode:"cors",credentials:"omit",cache:t?"force-cache":"default"});if(e.status!=200)throw new Error(e.status+" Unable to load "+e.url);return e}async function It(E,t){const e=E.body.getReader(),A=E.headers.get("content-length"),Q=A&&!isNaN(parseInt(A))?parseInt(A):void 0,i=[];let n=0;for(;;){const{done:s,value:C}=await e.read();if(s)break;if(i.push(C),n+=C.length,t&&Q){const d=n/Q,B=Math.min(d*.95,.95);t(B)}}const U=new Uint8Array(n);let F=0;for(const s of i)U.set(s,F),F+=s.length;return t&&t(1),U}class yt{static async LoadAsync(t,e,A,Q=!1){const i=await Ct(t,Q),n=await It(i,A);return this.LoadFromArrayBuffer(n.buffer,e)}static async LoadFromFileAsync(t,e,A){const Q=new FileReader;let i=new H;return Q.onload=n=>{i=this.LoadFromArrayBuffer(n.target.result,e)},Q.onprogress=n=>{A?.(n.loaded/n.total)},Q.readAsArrayBuffer(t),await new Promise(n=>{Q.onloadend=()=>{n()}}),i}static LoadFromArrayBuffer(t,e){const A=new Uint8Array(t),Q=v.Deserialize(A),i=new H(Q);return e.addObject(i),i}}class wt{static async LoadAsync(t,e,A,Q="",i=!1){const n=await Ct(t,i),U=await It(n,A);if(U[0]!==112||U[1]!==108||U[2]!==121||U[3]!==10)throw new Error("Invalid PLY file");return this.LoadFromArrayBuffer(U.buffer,e,Q)}static async LoadFromFileAsync(t,e,A,Q=""){const i=new FileReader;let n=new H;return i.onload=U=>{n=this.LoadFromArrayBuffer(U.target.result,e,Q)},i.onprogress=U=>{A?.(U.loaded/U.total)},i.readAsArrayBuffer(t),await new Promise(U=>{i.onloadend=()=>{U()}}),n}static LoadFromArrayBuffer(t,e,A=""){const Q=new Uint8Array(this._ParsePLYBuffer(t,A)),i=v.Deserialize(Q),n=new H(i);return e.addObject(n),n}static _ParsePLYBuffer(t,e){const A=new Uint8Array(t),Q=new TextDecoder().decode(A.slice(0,1024*10)),i=`end_header
`,n=Q.indexOf(i);if(n<0)throw new Error("Unable to read .ply file header");const U=parseInt(/element vertex (\d+)\n/.exec(Q)[1]);let F=0;const s={double:8,int:4,uint:4,float:4,short:2,ushort:2,uchar:1},C=[];for(const I of Q.slice(0,n).split(`
`).filter(h=>h.startsWith("property "))){const[h,a,l]=I.split(" ");if(C.push({name:l,type:a,offset:F}),!s[a])throw new Error(`Unsupported property type: ${a}`);F+=s[a]}const d=new DataView(t,n+i.length),B=new ArrayBuffer(v.RowLength*U),r=f.FromEuler(new R(Math.PI/2,0,0));for(let I=0;I<U;I++){const h=new Float32Array(B,I*v.RowLength,3),a=new Float32Array(B,I*v.RowLength+12,3),l=new Uint8ClampedArray(B,I*v.RowLength+24,4),c=new Uint8ClampedArray(B,I*v.RowLength+28,4);let u=255,w=0,D=0,k=0;C.forEach(y=>{let V;switch(y.type){case"float":V=d.getFloat32(y.offset+I*F,!0);break;case"int":V=d.getInt32(y.offset+I*F,!0);break;default:throw new Error(`Unsupported property type: ${y.type}`)}switch(y.name){case"x":h[0]=V;break;case"y":h[1]=V;break;case"z":h[2]=V;break;case"scale_0":case"scaling_0":a[0]=Math.exp(V);break;case"scale_1":case"scaling_1":a[1]=Math.exp(V);break;case"scale_2":case"scaling_2":a[2]=Math.exp(V);break;case"red":l[0]=V;break;case"green":l[1]=V;break;case"blue":l[2]=V;break;case"f_dc_0":case"features_0":l[0]=(.5+et.SH_C0*V)*255;break;case"f_dc_1":case"features_1":l[1]=(.5+et.SH_C0*V)*255;break;case"f_dc_2":case"features_2":l[2]=(.5+et.SH_C0*V)*255;break;case"f_dc_3":l[3]=(.5+et.SH_C0*V)*255;break;case"opacity":case"opacity_0":l[3]=1/(1+Math.exp(-V))*255;break;case"rot_0":case"rotation_0":u=V;break;case"rot_1":case"rotation_1":w=V;break;case"rot_2":case"rotation_2":D=V;break;case"rot_3":case"rotation_3":k=V;break}});let N=new f(w,D,k,u);switch(e){case"polycam":{const y=h[1];h[1]=-h[2],h[2]=y,N=r.multiply(N);break}case"":break;default:throw new Error(`Unsupported format: ${e}`)}N=N.normalize(),c[0]=N.w*128+128,c[1]=N.x*128+128,c[2]=N.y*128+128,c[3]=N.z*128+128}return B}}const lt="",Tt=E=>Uint8Array.from(atob(E),t=>t.charCodeAt(0)),Bt=typeof self<"u"&&self.Blob&&new Blob(["URL.revokeObjectURL(import.meta.url);",Tt(lt)],{type:"text/javascript;charset=utf-8"});function bt(E){let t;try{if(t=Bt&&(self.URL||self.webkitURL).createObjectURL(Bt),!t)throw"";const e=new Worker(t,{type:"module",name:E?.name});return e.addEventListener("error",()=>{(self.URL||self.webkitURL).revokeObjectURL(t)}),e}catch{return new Worker("data:text/javascript;base64,"+lt,{type:"module",name:E?.name})}}class Gt{constructor(t,e){this._scene=null,this._camera=null,this._started=!1,this._initialized=!1,this._renderer=t;const A=t.gl;this._program=A.createProgram(),this._passes=e||[];const Q=A.createShader(A.VERTEX_SHADER);A.shaderSource(Q,this._getVertexSource()),A.compileShader(Q),A.getShaderParameter(Q,A.COMPILE_STATUS)||console.error(A.getShaderInfoLog(Q));const i=A.createShader(A.FRAGMENT_SHADER);A.shaderSource(i,this._getFragmentSource()),A.compileShader(i),A.getShaderParameter(i,A.COMPILE_STATUS)||console.error(A.getShaderInfoLog(i)),A.attachShader(this.program,Q),A.attachShader(this.program,i),A.linkProgram(this.program),A.getProgramParameter(this.program,A.LINK_STATUS)||console.error(A.getProgramInfoLog(this.program)),this.resize=()=>{A.useProgram(this._program),this._resize()},this.initialize=()=>{console.assert(!this._initialized,"ShaderProgram already initialized"),A.useProgram(this._program),this._initialize();for(const n of this.passes)n.initialize(this);this._initialized=!0,this._started=!0},this.render=(n,U)=>{A.useProgram(this._program),(this._scene!==n||this._camera!==U)&&(this.dispose(),this._scene=n,this._camera=U,this.initialize());for(const F of this.passes)F.render();this._render()},this.dispose=()=>{if(this._initialized){A.useProgram(this._program);for(const n of this.passes)n.dispose();this._dispose(),this._scene=null,this._camera=null,this._initialized=!1}}}get renderer(){return this._renderer}get scene(){return this._scene}get camera(){return this._camera}get program(){return this._program}get passes(){return this._passes}get started(){return this._started}}const dt="",xt=E=>Uint8Array.from(atob(E),t=>t.charCodeAt(0)),ot=typeof self<"u"&&self.Blob&&new Blob(["URL.revokeObjectURL(import.meta.url);",xt(dt)],{type:"text/javascript;charset=utf-8"});function Mt(E){let t;try{if(t=ot&&(self.URL||self.webkitURL).createObjectURL(ot),!t)throw"";const e=new Worker(t,{type:"module",name:E?.name});return e.addEventListener("error",()=>{(self.URL||self.webkitURL).revokeObjectURL(t)}),e}catch{return new Worker("data:text/javascript;base64,"+dt,{type:"module",name:E?.name})}}var Lt=function(E={}){var t,e=E,A=import.meta.url,Q="",i;{try{Q=new URL(".",A).href}catch{}i=g=>{var J=new XMLHttpRequest;return J.open("GET",g,!1),J.responseType="arraybuffer",J.send(null),new Uint8Array(J.response)}}console.log.bind(console),console.error.bind(console);var n,U,F;function s(){var g=U.buffer;e.HEAPU8=F=new Uint8Array(g),e.HEAPU32=new Uint32Array(g),e.HEAPF32=new Float32Array(g),new BigInt64Array(g),new BigUint64Array(g)}function C(){if(e.preRun)for(typeof e.preRun=="function"&&(e.preRun=[e.preRun]);e.preRun.length;)M(e.preRun.shift());N(z)}function d(){j.c()}function B(){if(e.postRun)for(typeof e.postRun=="function"&&(e.postRun=[e.postRun]);e.postRun.length;)V(e.postRun.shift());N(y)}var r=0,I=null;function h(g){r++,e.monitorRunDependencies?.(r)}function a(g){if(r--,e.monitorRunDependencies?.(r),r==0&&I){var J=I;I=null,J()}}var l;function c(){return x("")}function u(g){if(ArrayBuffer.isView(g))return g;if(g==l&&n)return new Uint8Array(n);if(i)return i(g);throw'sync fetching of the wasm failed: you can preload it to Module["wasmBinary"] manually, or emcc.py will do that for you when generating HTML (but not JS)'}function w(g,J){var G,m=u(g);G=new WebAssembly.Module(m);var K=new WebAssembly.Instance(G,J);return[K,G]}function D(){return{a:b}}function k(){function g(m,K){return j=m.exports,U=j.b,s(),W(j),a(),j}h();var J=D();if(e.instantiateWasm)return new Promise((m,K)=>{e.instantiateWasm(J,(q,tt)=>{m(g(q))})});l??=c();var G=w(l,J);return g(G[0])}for(var N=g=>{for(;g.length>0;)g.shift()(e)},y=[],V=g=>y.push(g),z=[],M=g=>z.push(g),x=g=>{for(var J,G,m=0,K=0,q=g.length,tt=new Uint8Array((q*3>>2)-(g[q-2]=="=")-(g[q-1]=="="));m<q;m+=4,K+=3)J=o[g.charCodeAt(m+1)],G=o[g.charCodeAt(m+2)],tt[K]=o[g.charCodeAt(m)]<<2|J>>4,tt[K+1]=J<<4|G>>2,tt[K+2]=G<<6|o[g.charCodeAt(m+3)];return tt},Z=()=>2147483648,p=(g,J)=>Math.ceil(g/J)*J,O=g=>{var J=U.buffer,G=(g-J.byteLength+65535)/65536|0;try{return U.grow(G),s(),1}catch{}},T=g=>{var J=F.length;g>>>=0;var G=Z();if(g>G)return!1;for(var m=1;m<=4;m*=2){var K=J*(1+.2/m);K=Math.min(K,g+100663296);var q=Math.min(G,p(Math.max(g,K),65536)),tt=O(q);if(tt)return!0}return!1},o=new Uint8Array(123),S=25;S>=0;--S)o[48+S]=52+S,o[65+S]=S,o[97+S]=26+S;o[43]=62,o[47]=63,e.noExitRuntime&&e.noExitRuntime,e.print&&e.print,e.printErr&&e.printErr,e.wasmBinary&&(n=e.wasmBinary),e.arguments&&e.arguments,e.thisProgram&&e.thisProgram;function W(g){e._pack=g.d,e._malloc=g.e,e._free=g.f}var b={a:T},j=k();function $(){if(r>0){I=$;return}if(C(),r>0){I=$;return}function g(){e.calledRun=!0,d(),e.onRuntimeInitialized?.(),B()}e.setStatus?(e.setStatus("Running..."),setTimeout(()=>{setTimeout(()=>e.setStatus(""),1),g()},1)):g()}function L(){if(e.preInit)for(typeof e.preInit=="function"&&(e.preInit=[e.preInit]);e.preInit.length>0;)e.preInit.shift()()}return L(),$(),t=e,t};const Yt=()=>new Mt;class rt{constructor(t){this.dataChanged=!1,this.transformsChanged=!1,this.colorTransformsChanged=!1,this._updating=new Set,this._dirty=new Set;let e=0,A=0;this._splatIndices=new Map,this._offsets=new Map;const Q=new Map;for(const B of t.objects)B instanceof H&&(this._splatIndices.set(B,A),this._offsets.set(B,e),Q.set(e,B),e+=B.data.vertexCount,A++);this._vertexCount=e,this._width=2048,this._height=Math.ceil(2*this.vertexCount/this.width),this._data=new Uint32Array(this.width*this.height*4),this._transformsWidth=5,this._transformsHeight=Q.size,this._transforms=new Float32Array(this._transformsWidth*this._transformsHeight*4),this._transformIndicesWidth=1024,this._transformIndicesHeight=Math.ceil(this.vertexCount/this._transformIndicesWidth),this._transformIndices=new Uint32Array(this._transformIndicesWidth*this._transformIndicesHeight),this._colorTransformsWidth=4,this._colorTransformsHeight=64,this._colorTransforms=new Float32Array(this._colorTransformsWidth*this._colorTransformsHeight*4),this._colorTransforms.fill(0),this._colorTransforms[0]=1,this._colorTransforms[5]=1,this._colorTransforms[10]=1,this._colorTransforms[15]=1,this._colorTransformIndicesWidth=1024,this._colorTransformIndicesHeight=Math.ceil(this.vertexCount/this._colorTransformIndicesWidth),this._colorTransformIndices=new Uint32Array(this._colorTransformIndicesWidth*this._colorTransformIndicesHeight),this.colorTransformIndices.fill(0),this._positions=new Float32Array(this.vertexCount*3),this._rotations=new Float32Array(this.vertexCount*4),this._scales=new Float32Array(this.vertexCount*3),this._worker=Yt();const i=B=>{const r=this._splatIndices.get(B);this._transforms.set(B.transform.buffer,r*20),this._transforms[r*20+16]=B.selected?1:0,B.positionChanged=!1,B.rotationChanged=!1,B.scaleChanged=!1,B.selectedChanged=!1,this.transformsChanged=!0},n=()=>{let B=!1;for(const h of this._splatIndices.keys())if(h.colorTransformChanged){B=!0;break}if(!B)return;const r=[new _];this._colorTransformIndices.fill(0);let I=1;for(const h of this._splatIndices.keys()){const a=this._offsets.get(h);for(const l of h.colorTransforms)r.includes(l)||(r.push(l),I++);for(const l of h.colorTransformsMap.keys()){const c=h.colorTransformsMap.get(l);this._colorTransformIndices[l+a]=c+I-1}h.colorTransformChanged=!1}for(let h=0;h<r.length;h++){const a=r[h];this._colorTransforms.set(a.buffer,h*16)}this.colorTransformsChanged=!0};this._worker.onmessage=B=>{if(B.data.response){const r=B.data.response,I=Q.get(r.offset);i(I),n();const h=this._splatIndices.get(I);for(let a=0;a<I.data.vertexCount;a++)this._transformIndices[r.offset+a]=h;this._data.set(r.data,r.offset*8),I.data.reattach(r.positions,r.rotations,r.scales,r.colors,r.selection),this._positions.set(r.worldPositions,r.offset*3),this._rotations.set(r.worldRotations,r.offset*4),this._scales.set(r.worldScales,r.offset*3),this._updating.delete(I),I.selectedChanged=!1,this.dataChanged=!0}};let U;async function F(){U=await Lt()}F();async function s(){for(;!U;)await new Promise(B=>setTimeout(B,0))}const C=B=>{if(!U){s().then(()=>{C(B)});return}i(B);const r=U._malloc(3*B.data.vertexCount*4),I=U._malloc(4*B.data.vertexCount*4),h=U._malloc(3*B.data.vertexCount*4),a=U._malloc(4*B.data.vertexCount),l=U._malloc(B.data.vertexCount),c=U._malloc(8*B.data.vertexCount*4),u=U._malloc(3*B.data.vertexCount*4),w=U._malloc(4*B.data.vertexCount*4),D=U._malloc(3*B.data.vertexCount*4);U.HEAPF32.set(B.data.positions,r/4),U.HEAPF32.set(B.data.rotations,I/4),U.HEAPF32.set(B.data.scales,h/4),U.HEAPU8.set(B.data.colors,a),U.HEAPU8.set(B.data.selection,l),U._pack(B.selected,B.data.vertexCount,r,I,h,a,l,c,u,w,D);const k=new Uint32Array(U.HEAPU32.buffer,c,B.data.vertexCount*8),N=new Float32Array(U.HEAPF32.buffer,u,B.data.vertexCount*3),y=new Float32Array(U.HEAPF32.buffer,w,B.data.vertexCount*4),V=new Float32Array(U.HEAPF32.buffer,D,B.data.vertexCount*3),z=this._splatIndices.get(B),M=this._offsets.get(B);for(let x=0;x<B.data.vertexCount;x++)this._transformIndices[M+x]=z;this._data.set(k,M*8),this._positions.set(N,M*3),this._rotations.set(y,M*4),this._scales.set(V,M*3),U._free(r),U._free(I),U._free(h),U._free(a),U._free(l),U._free(c),U._free(u),U._free(w),U._free(D),this.dataChanged=!0,this.colorTransformsChanged=!0},d=B=>{if((B.positionChanged||B.rotationChanged||B.scaleChanged||B.selectedChanged)&&i(B),B.colorTransformChanged&&n(),!B.data.changed||B.data.detached)return;const r={position:new Float32Array(B.position.flat()),rotation:new Float32Array(B.rotation.flat()),scale:new Float32Array(B.scale.flat()),selected:B.selected,vertexCount:B.data.vertexCount,positions:B.data.positions,rotations:B.data.rotations,scales:B.data.scales,colors:B.data.colors,selection:B.data.selection,offset:this._offsets.get(B)};this._worker.postMessage({splat:r},[r.position.buffer,r.rotation.buffer,r.scale.buffer,r.positions.buffer,r.rotations.buffer,r.scales.buffer,r.colors.buffer,r.selection.buffer]),this._updating.add(B),B.data.detached=!0};this.getSplat=B=>{let r=null;for(const[I,h]of this._offsets)if(B>=h)r=I;else break;return r},this.getLocalIndex=(B,r)=>{const I=this._offsets.get(B);return r-I},this.markDirty=B=>{this._dirty.add(B)},this.rebuild=()=>{for(const B of this._dirty)d(B);this._dirty.clear()},this.dispose=()=>{this._worker.terminate()};for(const B of this._splatIndices.keys())C(B);n()}get offsets(){return this._offsets}get data(){return this._data}get width(){return this._width}get height(){return this._height}get transforms(){return this._transforms}get transformsWidth(){return this._transformsWidth}get transformsHeight(){return this._transformsHeight}get transformIndices(){return this._transformIndices}get transformIndicesWidth(){return this._transformIndicesWidth}get transformIndicesHeight(){return this._transformIndicesHeight}get colorTransforms(){return this._colorTransforms}get colorTransformsWidth(){return this._colorTransformsWidth}get colorTransformsHeight(){return this._colorTransformsHeight}get colorTransformIndices(){return this._colorTransformIndices}get colorTransformIndicesWidth(){return this._colorTransformIndicesWidth}get colorTransformIndicesHeight(){return this._colorTransformIndicesHeight}get positions(){return this._positions}get rotations(){return this._rotations}get scales(){return this._scales}get vertexCount(){return this._vertexCount}get needsRebuild(){return this._dirty.size>0}get updating(){return this._updating.size>0}}class ht{constructor(t=0,e=0,A=0,Q=255){this.r=t,this.g=e,this.b=A,this.a=Q}flat(){return[this.r,this.g,this.b,this.a]}flatNorm(){return[this.r/255,this.g/255,this.b/255,this.a/255]}toHexString(){return"#"+this.flat().map(t=>t.toString(16).padStart(2,"0")).join("")}toString(){return`[${this.flat().join(", ")}]`}}const Kt=()=>new bt,vt=`#version 300 es
precision highp float;
precision highp int;

uniform highp usampler2D u_texture;
uniform highp sampler2D u_transforms;
uniform highp usampler2D u_transformIndices;
uniform highp sampler2D u_colorTransforms;
uniform highp usampler2D u_colorTransformIndices;
uniform mat4 projection, view;
uniform vec2 focal;
uniform vec2 viewport;

uniform bool useDepthFade;
uniform float depthFade;

in vec2 position;
in int index;

out vec4 vColor;
out vec2 vPosition;
out float vSize;
out float vSelected;

void main () {
    uvec4 cen = texelFetch(u_texture, ivec2((uint(index) & 0x3ffu) << 1, uint(index) >> 10), 0);
    float selected = float((cen.w >> 24) & 0xffu);

    uint transformIndex = texelFetch(u_transformIndices, ivec2(uint(index) & 0x3ffu, uint(index) >> 10), 0).x;
    mat4 transform = mat4(
        texelFetch(u_transforms, ivec2(0, transformIndex), 0),
        texelFetch(u_transforms, ivec2(1, transformIndex), 0),
        texelFetch(u_transforms, ivec2(2, transformIndex), 0),
        texelFetch(u_transforms, ivec2(3, transformIndex), 0)
    );

    if (selected < 0.5) {
        selected = texelFetch(u_transforms, ivec2(4, transformIndex), 0).x;
    }

    mat4 viewTransform = view * transform;

    vec4 cam = viewTransform * vec4(uintBitsToFloat(cen.xyz), 1);
    vec4 pos2d = projection * cam;

    float clip = 1.2 * pos2d.w;
    if (pos2d.z < -pos2d.w || pos2d.z > pos2d.w || pos2d.x < -clip || pos2d.x > clip || pos2d.y < -clip || pos2d.y > clip) {
        gl_Position = vec4(0.0, 0.0, 2.0, 1.0);
        return;
    }

    uvec4 cov = texelFetch(u_texture, ivec2(((uint(index) & 0x3ffu) << 1) | 1u, uint(index) >> 10), 0);
    vec2 u1 = unpackHalf2x16(cov.x), u2 = unpackHalf2x16(cov.y), u3 = unpackHalf2x16(cov.z);
    mat3 Vrk = mat3(u1.x, u1.y, u2.x, u1.y, u2.y, u3.x, u2.x, u3.x, u3.y);

    mat3 J = mat3(
        focal.x / cam.z, 0., -(focal.x * cam.x) / (cam.z * cam.z), 
        0., -focal.y / cam.z, (focal.y * cam.y) / (cam.z * cam.z), 
        0., 0., 0.
    );

    mat3 T = transpose(mat3(viewTransform)) * J;
    mat3 cov2d = transpose(T) * Vrk * T;

    //ref: https://github.com/graphdeco-inria/diff-gaussian-rasterization/blob/main/cuda_rasterizer/forward.cu#L110-L111
    cov2d[0][0] += 0.3;
    cov2d[1][1] += 0.3;

    float mid = (cov2d[0][0] + cov2d[1][1]) / 2.0;
    float radius = length(vec2((cov2d[0][0] - cov2d[1][1]) / 2.0, cov2d[0][1]));
    float lambda1 = mid + radius, lambda2 = mid - radius;

    if (lambda2 < 0.0) return;
    vec2 diagonalVector = normalize(vec2(cov2d[0][1], lambda1 - cov2d[0][0]));
    vec2 majorAxis = min(sqrt(2.0 * lambda1), 1024.0) * diagonalVector;
    vec2 minorAxis = min(sqrt(2.0 * lambda2), 1024.0) * vec2(diagonalVector.y, -diagonalVector.x);

    uint colorTransformIndex = texelFetch(u_colorTransformIndices, ivec2(uint(index) & 0x3ffu, uint(index) >> 10), 0).x;
    mat4 colorTransform = mat4(
        texelFetch(u_colorTransforms, ivec2(0, colorTransformIndex), 0),
        texelFetch(u_colorTransforms, ivec2(1, colorTransformIndex), 0),
        texelFetch(u_colorTransforms, ivec2(2, colorTransformIndex), 0),
        texelFetch(u_colorTransforms, ivec2(3, colorTransformIndex), 0)
    );

    vec4 color = vec4((cov.w) & 0xffu, (cov.w >> 8) & 0xffu, (cov.w >> 16) & 0xffu, (cov.w >> 24) & 0xffu) / 255.0;
    vColor = colorTransform * color;

    vPosition = position;
    vSize = length(majorAxis);
    vSelected = selected;

    float scalingFactor = 1.0;

    if (useDepthFade) {
        float depthNorm = (pos2d.z / pos2d.w + 1.0) / 2.0;
        float near = 0.1; float far = 100.0;
        float normalizedDepth = (2.0 * near) / (far + near - depthNorm * (far - near));
        float start = max(normalizedDepth - 0.1, 0.0);
        float end = min(normalizedDepth + 0.1, 1.0);
        scalingFactor = clamp((depthFade - start) / (end - start), 0.0, 1.0);
    }

    vec2 vCenter = vec2(pos2d) / pos2d.w;
    gl_Position = vec4(
        vCenter 
        + position.x * majorAxis * scalingFactor / viewport
        + position.y * minorAxis * scalingFactor / viewport, 0.0, 1.0);
}
`,Xt=`#version 300 es
precision highp float;

uniform float outlineThickness;
uniform vec4 outlineColor;

in vec4 vColor;
in vec2 vPosition;
in float vSize;
in float vSelected;

out vec4 fragColor;

void main () {
    float A = -dot(vPosition, vPosition);

    if (A < -4.0) discard;

    if (vSelected < 0.5) {
        float B = exp(A) * vColor.a;
        fragColor = vec4(B * vColor.rgb, B);
        return;
    }

    float outlineThreshold = -4.0 + (outlineThickness / vSize);

    if (A < outlineThreshold) {
        fragColor = outlineColor;
    } 
    else {
        float B = exp(A) * vColor.a;
        fragColor = vec4(B * vColor.rgb, B);
    }
}
`;class Rt extends Gt{constructor(t,e){super(t,e),this._outlineThickness=10,this._outlineColor=new ht(255,165,0,255),this._renderData=null,this._depthIndex=new Uint32Array,this._splatTexture=null,this._worker=null;const A=t.canvas,Q=t.gl;let i,n,U,F,s,C,d,B,r,I,h,a,l,c,u,w,D,k,N;this._resize=()=>{this._camera&&(this._camera.data.setSize(A.width,A.height),this._camera.update(),i=Q.getUniformLocation(this.program,"projection"),Q.uniformMatrix4fv(i,!1,this._camera.data.projectionMatrix.buffer),n=Q.getUniformLocation(this.program,"viewport"),Q.uniform2fv(n,new Float32Array([A.width,A.height])))};const y=()=>{this._worker=Kt(),this._worker.onmessage=Z=>{if(Z.data.depthIndex){const{depthIndex:p}=Z.data;this._depthIndex=p,Q.bindBuffer(Q.ARRAY_BUFFER,N),Q.bufferData(Q.ARRAY_BUFFER,p,Q.STATIC_DRAW)}}};this._initialize=()=>{if(!this._scene||!this._camera){console.error("Cannot render without scene and camera");return}this._resize(),this._scene.addEventListener("objectAdded",V),this._scene.addEventListener("objectRemoved",z);for(const Z of this._scene.objects)Z instanceof H&&Z.addEventListener("objectChanged",M);this._renderData=new rt(this._scene),U=Q.getUniformLocation(this.program,"focal"),Q.uniform2fv(U,new Float32Array([this._camera.data.fx,this._camera.data.fy])),F=Q.getUniformLocation(this.program,"view"),Q.uniformMatrix4fv(F,!1,this._camera.data.viewMatrix.buffer),I=Q.getUniformLocation(this.program,"outlineThickness"),Q.uniform1f(I,this.outlineThickness),h=Q.getUniformLocation(this.program,"outlineColor"),Q.uniform4fv(h,new Float32Array(this.outlineColor.flatNorm())),this._splatTexture=Q.createTexture(),s=Q.getUniformLocation(this.program,"u_texture"),Q.uniform1i(s,0),c=Q.createTexture(),C=Q.getUniformLocation(this.program,"u_transforms"),Q.uniform1i(C,1),u=Q.createTexture(),d=Q.getUniformLocation(this.program,"u_transformIndices"),Q.uniform1i(d,2),w=Q.createTexture(),B=Q.getUniformLocation(this.program,"u_colorTransforms"),Q.uniform1i(B,3),D=Q.createTexture(),r=Q.getUniformLocation(this.program,"u_colorTransformIndices"),Q.uniform1i(r,4),k=Q.createBuffer(),Q.bindBuffer(Q.ARRAY_BUFFER,k),Q.bufferData(Q.ARRAY_BUFFER,new Float32Array([-2,-2,2,-2,2,2,-2,2]),Q.STATIC_DRAW),a=Q.getAttribLocation(this.program,"position"),Q.enableVertexAttribArray(a),Q.vertexAttribPointer(a,2,Q.FLOAT,!1,0,0),N=Q.createBuffer(),l=Q.getAttribLocation(this.program,"index"),Q.enableVertexAttribArray(l),Q.bindBuffer(Q.ARRAY_BUFFER,N),y()};const V=Z=>{const p=Z;p.object instanceof H&&p.object.addEventListener("objectChanged",M),x()},z=Z=>{const p=Z;p.object instanceof H&&p.object.removeEventListener("objectChanged",M),x()},M=Z=>{const p=Z;p.object instanceof H&&this._renderData&&this._renderData.markDirty(p.object)},x=()=>{this._renderData?.dispose(),this._renderData=new rt(this._scene),this._worker?.terminate(),y()};this._render=()=>{if(!this._scene||!this._camera||!this.renderData){console.error("Cannot render without scene and camera");return}if(this.renderData.needsRebuild&&this.renderData.rebuild(),this.renderData.dataChanged||this.renderData.transformsChanged||this.renderData.colorTransformsChanged){this.renderData.dataChanged&&(Q.activeTexture(Q.TEXTURE0),Q.bindTexture(Q.TEXTURE_2D,this.splatTexture),Q.texParameteri(Q.TEXTURE_2D,Q.TEXTURE_WRAP_S,Q.CLAMP_TO_EDGE),Q.texParameteri(Q.TEXTURE_2D,Q.TEXTURE_WRAP_T,Q.CLAMP_TO_EDGE),Q.texParameteri(Q.TEXTURE_2D,Q.TEXTURE_MIN_FILTER,Q.NEAREST),Q.texParameteri(Q.TEXTURE_2D,Q.TEXTURE_MAG_FILTER,Q.NEAREST),Q.texImage2D(Q.TEXTURE_2D,0,Q.RGBA32UI,this.renderData.width,this.renderData.height,0,Q.RGBA_INTEGER,Q.UNSIGNED_INT,this.renderData.data)),this.renderData.transformsChanged&&(Q.activeTexture(Q.TEXTURE1),Q.bindTexture(Q.TEXTURE_2D,c),Q.texParameteri(Q.TEXTURE_2D,Q.TEXTURE_WRAP_S,Q.CLAMP_TO_EDGE),Q.texParameteri(Q.TEXTURE_2D,Q.TEXTURE_WRAP_T,Q.CLAMP_TO_EDGE),Q.texParameteri(Q.TEXTURE_2D,Q.TEXTURE_MIN_FILTER,Q.NEAREST),Q.texParameteri(Q.TEXTURE_2D,Q.TEXTURE_MAG_FILTER,Q.NEAREST),Q.texImage2D(Q.TEXTURE_2D,0,Q.RGBA32F,this.renderData.transformsWidth,this.renderData.transformsHeight,0,Q.RGBA,Q.FLOAT,this.renderData.transforms),Q.activeTexture(Q.TEXTURE2),Q.bindTexture(Q.TEXTURE_2D,u),Q.texParameteri(Q.TEXTURE_2D,Q.TEXTURE_WRAP_S,Q.CLAMP_TO_EDGE),Q.texParameteri(Q.TEXTURE_2D,Q.TEXTURE_WRAP_T,Q.CLAMP_TO_EDGE),Q.texParameteri(Q.TEXTURE_2D,Q.TEXTURE_MIN_FILTER,Q.NEAREST),Q.texParameteri(Q.TEXTURE_2D,Q.TEXTURE_MAG_FILTER,Q.NEAREST),Q.texImage2D(Q.TEXTURE_2D,0,Q.R32UI,this.renderData.transformIndicesWidth,this.renderData.transformIndicesHeight,0,Q.RED_INTEGER,Q.UNSIGNED_INT,this.renderData.transformIndices)),this.renderData.colorTransformsChanged&&(Q.activeTexture(Q.TEXTURE3),Q.bindTexture(Q.TEXTURE_2D,w),Q.texParameteri(Q.TEXTURE_2D,Q.TEXTURE_WRAP_S,Q.CLAMP_TO_EDGE),Q.texParameteri(Q.TEXTURE_2D,Q.TEXTURE_WRAP_T,Q.CLAMP_TO_EDGE),Q.texParameteri(Q.TEXTURE_2D,Q.TEXTURE_MIN_FILTER,Q.NEAREST),Q.texParameteri(Q.TEXTURE_2D,Q.TEXTURE_MAG_FILTER,Q.NEAREST),Q.texImage2D(Q.TEXTURE_2D,0,Q.RGBA32F,this.renderData.colorTransformsWidth,this.renderData.colorTransformsHeight,0,Q.RGBA,Q.FLOAT,this.renderData.colorTransforms),Q.activeTexture(Q.TEXTURE4),Q.bindTexture(Q.TEXTURE_2D,D),Q.texParameteri(Q.TEXTURE_2D,Q.TEXTURE_WRAP_S,Q.CLAMP_TO_EDGE),Q.texParameteri(Q.TEXTURE_2D,Q.TEXTURE_WRAP_T,Q.CLAMP_TO_EDGE),Q.texParameteri(Q.TEXTURE_2D,Q.TEXTURE_MIN_FILTER,Q.NEAREST),Q.texParameteri(Q.TEXTURE_2D,Q.TEXTURE_MAG_FILTER,Q.NEAREST),Q.texImage2D(Q.TEXTURE_2D,0,Q.R32UI,this.renderData.colorTransformIndicesWidth,this.renderData.colorTransformIndicesHeight,0,Q.RED_INTEGER,Q.UNSIGNED_INT,this.renderData.colorTransformIndices));const Z=new Float32Array(this.renderData.positions.slice().buffer),p=new Float32Array(this.renderData.transforms.slice().buffer),O=new Uint32Array(this.renderData.transformIndices.slice().buffer);this._worker?.postMessage({sortData:{positions:Z,transforms:p,transformIndices:O,vertexCount:this.renderData.vertexCount}},[Z.buffer,p.buffer,O.buffer]),this.renderData.dataChanged=!1,this.renderData.transformsChanged=!1,this.renderData.colorTransformsChanged=!1}this._camera.update(),this._worker?.postMessage({viewProj:this._camera.data.viewProj.buffer}),Q.viewport(0,0,A.width,A.height),Q.clearColor(0,0,0,0),Q.clear(Q.COLOR_BUFFER_BIT),Q.disable(Q.DEPTH_TEST),Q.enable(Q.BLEND),Q.blendFuncSeparate(Q.ONE_MINUS_DST_ALPHA,Q.ONE,Q.ONE_MINUS_DST_ALPHA,Q.ONE),Q.blendEquationSeparate(Q.FUNC_ADD,Q.FUNC_ADD),Q.uniformMatrix4fv(i,!1,this._camera.data.projectionMatrix.buffer),Q.uniformMatrix4fv(F,!1,this._camera.data.viewMatrix.buffer),Q.bindBuffer(Q.ARRAY_BUFFER,k),Q.vertexAttribPointer(a,2,Q.FLOAT,!1,0,0),Q.bindBuffer(Q.ARRAY_BUFFER,N),Q.bufferData(Q.ARRAY_BUFFER,this.depthIndex,Q.STATIC_DRAW),Q.vertexAttribIPointer(l,1,Q.INT,0,0),Q.vertexAttribDivisor(l,1),Q.drawArraysInstanced(Q.TRIANGLE_FAN,0,4,this.depthIndex.length)},this._dispose=()=>{if(!this._scene||!this._camera||!this.renderData){console.error("Cannot dispose without scene and camera");return}this._scene.removeEventListener("objectAdded",V),this._scene.removeEventListener("objectRemoved",z);for(const Z of this._scene.objects)Z instanceof H&&Z.removeEventListener("objectChanged",M);this._worker?.terminate(),this.renderData.dispose(),Q.deleteTexture(this.splatTexture),Q.deleteTexture(c),Q.deleteTexture(u),Q.deleteBuffer(N),Q.deleteBuffer(k)},this._setOutlineThickness=Z=>{this._outlineThickness=Z,this._initialized&&Q.uniform1f(I,Z)},this._setOutlineColor=Z=>{this._outlineColor=Z,this._initialized&&Q.uniform4fv(h,new Float32Array(Z.flatNorm()))}}get renderData(){return this._renderData}get depthIndex(){return this._depthIndex}get splatTexture(){return this._splatTexture}get outlineThickness(){return this._outlineThickness}set outlineThickness(t){this._setOutlineThickness(t)}get outlineColor(){return this._outlineColor}set outlineColor(t){this._setOutlineColor(t)}get worker(){return this._worker}_getVertexSource(){return vt}_getFragmentSource(){return Xt}}class _t{constructor(t=1){let e=0,A=!1,Q,i,n,U;this.initialize=F=>{if(!(F instanceof Rt))throw new Error("FadeInPass requires a RenderProgram");e=F.started?1:0,A=!0,Q=F,i=F.renderer.gl,n=i.getUniformLocation(Q.program,"useDepthFade"),i.uniform1i(n,1),U=i.getUniformLocation(Q.program,"depthFade"),i.uniform1f(U,e)},this.render=()=>{!A||Q.renderData?.updating||(i.useProgram(Q.program),e=Math.min(e+t*.01,1),e>=1&&(A=!1,i.uniform1i(n,0)),i.uniform1f(U,e))}}dispose(){}}class Ht{constructor(t=null,e=null){this._backgroundColor=new ht;const A=t||document.createElement("canvas");t||(A.style.display="block",A.style.boxSizing="border-box",A.style.width="100%",A.style.height="100%",A.style.margin="0",A.style.padding="0",document.body.appendChild(A)),A.style.background=this._backgroundColor.toHexString(),this._canvas=A,this._gl=A.getContext("webgl2",{antialias:!1});const Q=e||[];e||Q.push(new _t),this._renderProgram=new Rt(this,Q);const i=[this._renderProgram];this.resize=()=>{const n=A.clientWidth,U=A.clientHeight;(A.width!==n||A.height!==U)&&this.setSize(n,U)},this.setSize=(n,U)=>{A.width=n,A.height=U,this._gl.viewport(0,0,A.width,A.height);for(const F of i)F.resize()},this.render=(n,U)=>{for(const F of i)F.render(n,U)},this.dispose=()=>{for(const n of i)n.dispose()},this.addProgram=n=>{i.push(n)},this.removeProgram=n=>{const U=i.indexOf(n);if(U<0)throw new Error("Program not found");i.splice(U,1)},this.resize()}get canvas(){return this._canvas}get gl(){return this._gl}get renderProgram(){return this._renderProgram}get backgroundColor(){return this._backgroundColor}set backgroundColor(t){this._backgroundColor=t,this._canvas.style.background=t.toHexString()}}class zt{constructor(t,e,A=.5,Q=.5,i=5,n=!0,U=new R){this.minAngle=-90,this.maxAngle=90,this.minZoom=.1,this.maxZoom=30,this.orbitSpeed=1,this.panSpeed=1,this.zoomSpeed=1,this.dampening=.12,this.setCameraTarget=()=>{};let F=U.clone(),s=F.clone(),C=A,d=Q,B=i,r=!1,I=!1,h=0,a=0,l=0;const c={};let u=!1;const w=()=>{if(u)return;const o=t.rotation.toEuler();C=-o.y,d=-o.x;const S=t.position.x-B*Math.sin(C)*Math.cos(d),W=t.position.y+B*Math.sin(d),b=t.position.z+B*Math.cos(C)*Math.cos(d);s=new R(S,W,b)};t.addEventListener("objectChanged",w),this.setCameraTarget=o=>{const S=o.x-t.position.x,W=o.y-t.position.y,b=o.z-t.position.z;B=Math.sqrt(S*S+W*W+b*b),d=Math.atan2(W,Math.sqrt(S*S+b*b)),C=-Math.atan2(S,b),s=new R(o.x,o.y,o.z)};const D=()=>.1+.9*(B-this.minZoom)/(this.maxZoom-this.minZoom),k=o=>{c[o.code]=!0,o.code==="ArrowUp"&&(c.KeyW=!0),o.code==="ArrowDown"&&(c.KeyS=!0),o.code==="ArrowLeft"&&(c.KeyA=!0),o.code==="ArrowRight"&&(c.KeyD=!0)},N=o=>{c[o.code]=!1,o.code==="ArrowUp"&&(c.KeyW=!1),o.code==="ArrowDown"&&(c.KeyS=!1),o.code==="ArrowLeft"&&(c.KeyA=!1),o.code==="ArrowRight"&&(c.KeyD=!1)},y=o=>{T(o),r=!0,I=o.button===2,a=o.clientX,l=o.clientY,window.addEventListener("mouseup",V)},V=o=>{T(o),r=!1,I=!1,window.removeEventListener("mouseup",V)},z=o=>{if(T(o),!r||!t)return;const S=o.clientX-a,W=o.clientY-l;if(I){const b=D(),j=-S*this.panSpeed*.01*b,$=-W*this.panSpeed*.01*b,L=P.RotationFromQuaternion(t.rotation).buffer,g=new R(L[0],L[3],L[6]),J=new R(L[1],L[4],L[7]);s=s.add(g.multiply(j)),s=s.add(J.multiply($))}else C-=S*this.orbitSpeed*.003,d+=W*this.orbitSpeed*.003,d=Math.min(Math.max(d,this.minAngle*Math.PI/180),this.maxAngle*Math.PI/180);a=o.clientX,l=o.clientY},M=o=>{T(o);const S=D();B+=o.deltaY*this.zoomSpeed*.025*S,B=Math.min(Math.max(B,this.minZoom),this.maxZoom)},x=o=>{if(T(o),o.touches.length===1)r=!0,I=!1,a=o.touches[0].clientX,l=o.touches[0].clientY,h=0;else if(o.touches.length===2){r=!0,I=!0,a=(o.touches[0].clientX+o.touches[1].clientX)/2,l=(o.touches[0].clientY+o.touches[1].clientY)/2;const S=o.touches[0].clientX-o.touches[1].clientX,W=o.touches[0].clientY-o.touches[1].clientY;h=Math.sqrt(S*S+W*W)}},Z=o=>{T(o),r=!1,I=!1},p=o=>{if(T(o),!(!r||!t))if(I){const S=D(),W=o.touches[0].clientX-o.touches[1].clientX,b=o.touches[0].clientY-o.touches[1].clientY,j=Math.sqrt(W*W+b*b),$=h-j;B+=$*this.zoomSpeed*.1*S,B=Math.min(Math.max(B,this.minZoom),this.maxZoom),h=j;const L=(o.touches[0].clientX+o.touches[1].clientX)/2,g=(o.touches[0].clientY+o.touches[1].clientY)/2,J=L-a,G=g-l,m=P.RotationFromQuaternion(t.rotation).buffer,K=new R(m[0],m[3],m[6]),q=new R(m[1],m[4],m[7]);s=s.add(K.multiply(-J*this.panSpeed*.025*S)),s=s.add(q.multiply(-G*this.panSpeed*.025*S)),a=L,l=g}else{const S=o.touches[0].clientX-a,W=o.touches[0].clientY-l;C-=S*this.orbitSpeed*.003,d+=W*this.orbitSpeed*.003,d=Math.min(Math.max(d,this.minAngle*Math.PI/180),this.maxAngle*Math.PI/180),a=o.touches[0].clientX,l=o.touches[0].clientY}},O=(o,S,W)=>(1-W)*o+W*S;this.update=()=>{u=!0,A=O(A,C,this.dampening),Q=O(Q,d,this.dampening),i=O(i,B,this.dampening),F=F.lerp(s,this.dampening);const o=F.x+i*Math.sin(A)*Math.cos(Q),S=F.y-i*Math.sin(Q),W=F.z-i*Math.cos(A)*Math.cos(Q);t.position=new R(o,S,W);const b=F.subtract(t.position).normalize(),j=Math.asin(-b.y),$=Math.atan2(b.x,b.z);t.rotation=f.FromEuler(new R(j,$,0));const L=.025,g=.01,J=P.RotationFromQuaternion(t.rotation).buffer,G=new R(-J[2],-J[5],-J[8]),m=new R(J[0],J[3],J[6]);c.KeyS&&(s=s.add(G.multiply(L))),c.KeyW&&(s=s.subtract(G.multiply(L))),c.KeyA&&(s=s.subtract(m.multiply(L))),c.KeyD&&(s=s.add(m.multiply(L))),c.KeyE&&(C+=g),c.KeyQ&&(C-=g),c.KeyR&&(d+=g),c.KeyF&&(d-=g),u=!1};const T=o=>{o.preventDefault(),o.stopPropagation()};this.dispose=()=>{e.removeEventListener("dragenter",T),e.removeEventListener("dragover",T),e.removeEventListener("dragleave",T),e.removeEventListener("contextmenu",T),e.removeEventListener("mousedown",y),e.removeEventListener("mousemove",z),e.removeEventListener("wheel",M),e.removeEventListener("touchstart",x),e.removeEventListener("touchend",Z),e.removeEventListener("touchmove",p),n&&(window.removeEventListener("keydown",k),window.removeEventListener("keyup",N))},n&&(window.addEventListener("keydown",k),window.addEventListener("keyup",N)),e.addEventListener("dragenter",T),e.addEventListener("dragover",T),e.addEventListener("dragleave",T),e.addEventListener("contextmenu",T),e.addEventListener("mousedown",y),e.addEventListener("mousemove",z),e.addEventListener("wheel",M),e.addEventListener("touchstart",x),e.addEventListener("touchend",Z),e.addEventListener("touchmove",p),this.update()}}var jt=Zt("<canvas></canvas>");function qt(E,t){St(t,!1);const e=At(),A=At();let Q=Ut(t,"value",12),i=Ut(t,"zoom_speed",12),n=Ut(t,"pan_speed",12),U=At(),F,s,C=null,d,B=At(!1),r=null;function I(){if(r!==null&&(cancelAnimationFrame(r),r=null),C!==null&&(C.dispose(),C=null),F=new kt,s=new ft,C=new Ht(Y(U)),d=new zt(s,Y(U)),d.zoomSpeed=i(),d.panSpeed=n(),!Q())return;let l=!1;const c=async()=>{if(l){console.error("Already loading");return}if(!Y(e))throw new Error("No resolved URL");if(l=!0,Y(e).endsWith(".ply"))await wt.LoadAsync(Y(e),F,void 0);else if(Y(e).endsWith(".splat"))await yt.LoadAsync(Y(e),F,void 0);else throw new Error("Unsupported file type");l=!1},u=()=>{if(C){if(l){r=requestAnimationFrame(u);return}d.update(),C.render(F,s),r=requestAnimationFrame(u)}};c(),r=requestAnimationFrame(u)}Jt(()=>(Q()!=null&&I(),Qt(B,!0),()=>{C&&C.dispose()})),nt(()=>it(Q()),()=>{Qt(e,Q().url)}),nt(()=>(Y(A),it(Q())),()=>{(l=>{Qt(A,l.path)})(Q()||{path:void 0})}),nt(()=>(Y(U),Y(B),Y(A)),()=>{Y(U)&&Y(B)&&Y(A)&&I()}),Vt();var h={get value(){return Q()},set value(l){Q(l),Ft()},get zoom_speed(){return i()},set zoom_speed(l){i(l),Ft()},get pan_speed(){return n()},set pan_speed(l){n(l),Ft()}};ct();var a=jt();return Et(a,l=>Qt(U,l),()=>Y(U)),mt(E,a),ut(h)}export{qt as default};
